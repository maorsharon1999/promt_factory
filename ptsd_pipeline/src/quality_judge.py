...
class JudgeOllamaClient:
    """
    Dedicated Ollama chat client for the G-Eval judge.

    Uses /api/chat (not /api/generate) so the system role is respected
    and llama3 replies in English JSON instead of echoing Hebrew.
    Falls back to plain generate if chat endpoint errors.
    """
    ...
    def _chat(self, system: str, user: str) -> str:
        ...
        payload = json.dumps(
            {
                ...
                "stream": False,
                "options": {"temperature": 0.1},  # low temp = deterministic JSON
            }
        ).encode()
        ...
        with _req.urlopen(request, timeout=self._timeout) as resp:
...
def judge_record(
    record: dict,
    client: "JudgeOllamaClient | LLMClient",
    cache: dict[str, dict],
) -> JudgeVerdict:
    """
...
def run_judge_pipeline(
    ...
    # Use JudgeOllamaClient (chat endpoint) for proper JSON output from llama3.
    # create_llm_client / ResilienceLLMClient are preserved for non-judge generation use.
    if mock:
        client: "JudgeOllamaClient | LLMClient" = create_llm_client(mock=True)
    else:
        client = JudgeOllamaClient(model="llama3", timeout=300)
